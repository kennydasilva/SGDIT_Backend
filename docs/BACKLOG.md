...
- **Geolocalização nas denúncias — 1ª fase concluída (escolha manual pelo cidadão):**
  ...
  - **2ª fase por fazer:** jurisdição de cada posto (Admin) como **polígono desenhado no mapa** (decisão do utilizador — cada posto tem uma área de responsabilidade, ex: "Mavalane" cobre Hulene/Xiquelene/Aeroporto, "12ª Esquadra" cobre Maxaquene/Praça dos Combatentes/Av. Vladimir Lenine). Precisa de: novo modelo para guardar o polígono por Admin; ferramenta de desenho no mapa (Google Maps Drawing Library) numa UI só para Admin/Super Admin; lógica de "ponto dentro do polígono" no backend para saber a que posto pertence uma denúncia. Isto é o que desbloqueia o "agente mais próximo" para o SMS de acidente de viação (já não é distância a PTs individuais, é "a que posto pertence esta zona").
